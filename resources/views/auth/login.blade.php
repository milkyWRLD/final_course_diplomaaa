      ...
      <div class="login__wrapper">
        <form class="login__form" action="{{ route('login')}}" method="POST" accept-charset="utf-8">
          ...
          <label class="login__label" for="email">
            E-mail
            <input class="login__input" id="email" type="email" placeholder="example@domain.ru" name="email" value="{{ old('email') }}" required autofocus>
          </label>
          <label class="login__label" for="pwd">
            Пароль
            <input class="login__input" id="password" type="password" placeholder="" name="password" required>
          </label>
